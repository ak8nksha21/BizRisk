...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  // Guard against a stalled backend response: fetch() has no default timeout,
  // so without this an unresponsive request leaves callers (e.g. the
  // dashboard's loading state) stuck indefinitely instead of surfacing an error.
  ...
  try {
    ...
    if (err instanceof DOMException && err.name === 'AbortError') {
      throw new APIError('Request timed out. Please check if the server is running.', 504);
    }
