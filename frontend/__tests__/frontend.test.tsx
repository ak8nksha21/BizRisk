...
describe('BizRisk Frontend Component Tests', () => {
  ...
  test('shows error message if API fails during launch', async () => {
    ...
    const nameInput = screen.getByLabelText(/Merchant \/ Legal Entity Name/i);
  ...
  test('renders investigation details, risk metrics, evidence, report findings, and QA PASS status', async () => {
    ...
    // Findings detail is collapsed by default on the redesigned decision
    // console -- expand it before asserting on its contents.
    fireEvent.click(screen.getByText(/Full report details/i));
