...
_MERCHANT_DECISION_BY_RISK_LEVEL = {
    "low": "APPROVE",
...
def determine_merchant_decision(
    risk_level: str | None,
    assessment_status: str | None,
    resolution_status: str | None,
) -> str:
    """
    Deterministic merchant onboarding decision, derived ONLY from the existing
    authoritative risk_level / assessment_status / entity resolution status.
    Never computed or overridden by the LLM narrative -- this must stay a pure
    function of already-final, deterministic pipeline state.
    ...
    Mapping:
      ...
      HIGH                                 -> MANUAL_REVIEW
      VERY_HIGH                            -> REJECT_OR_ESCALATE
      INSUFFICIENT_EVIDENCE / unresolved identity -> MANUAL_REVIEW

    "Unresolved identity" here means ``resolution_status == "CONFLICTING_IDENTITY"``
    (app/entity_resolution/resolver.py): the target's own statutory identifier
    (GSTIN/CIN) actively conflicts with a discovered candidate's -- the
    "conflicting business identity information" this product exists to catch.
    Plain ``"ENTITY_UNRESOLVED"`` is deliberately NOT treated as an override:
    it is also the ordinary, non-suspicious result whenever no discovery
    candidates were produced at all (e.g. a direct GSTIN/CIN lookup with a
    single source), which is the common case and must remain free to reach
    APPROVE on a genuinely LOW deterministic risk score.
    """
    if assessment_status == "INSUFFICIENT_EVIDENCE":
        return "MANUAL_REVIEW"
    if resolution_status == "CONFLICTING_IDENTITY":
        return "MANUAL_REVIEW"
    return _MERCHANT_DECISION_BY_RISK_LEVEL.get(
        str(risk_level or "").strip().lower(), "MANUAL_REVIEW"
    )
...
def generate_investigation_report(
    ...
    with db_lock:
        ...
        merchant_decision = determine_merchant_decision(
            analysis["overall_risk"]["level"],
            assessment_status,
