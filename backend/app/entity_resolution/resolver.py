...
def resolve_entity(
    ...
    # GAP C: a candidate that merely restates the target's own claimed identity
    # (entity discovery's "intake_echo" candidate -- see app/agents/discovery.py)
    # provides no independent corroboration. If it is the ONLY reason we'd
    # resolve cleanly while a genuinely different, independently discovered
    # candidate conflicts on a strong statutory identifier, the echo must not
    # be allowed to bury that conflict. A non-echo candidate winning the exact
    # match (e.g. GAP A's own passing case, where a different candidate's own
    # matching CIN legitimately corroborates) is unaffected.
    non_echo_exact_matches = [c for c in exact_matches if c.get("source") != "intake_echo"]
    if not non_echo_exact_matches and exact_matches and conflicts:
        return {
            "entity": None,
            "confidence": 0.0,
            "matched": False,
            ...
            "match_reasons": [
                "Identifier conflict raised by an independently discovered candidate; "
                "the only clean match is a restatement of the target's own claimed identity."
            ],
            "conflicting_identifiers": conflicts,
        }

    if exact_matches:
        matched_cand = non_echo_exact_matches[0] if non_echo_exact_matches else exact_matches[0]
